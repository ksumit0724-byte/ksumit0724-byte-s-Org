...
export default function ModeSelection() {
  const { setMode, updateUser } = useAetherStore();
  ...
  const handleSelect = async (mode: AppMode) => {
    ...
    if (client) {
      const { error } = await client.auth.updateUser({
        data: { modeSelected: true, defaultMode: mode }
      });

      if (!error) {
        updateUser({ modeSelected: true, defaultMode: mode });
      }
    } else {
      console.log("[DemoMode] Updating user state...");
      updateUser({ modeSelected: true, defaultMode: mode });
    }
  ...
  return (
    <div className="h-screen w-full flex flex-col items-center justify-center p-6 gap-12 relative overflow-hidden">
      ...
      <div className="z-10 text-center space-y-4 px-4 overflow-hidden">
        <h2 className="text-3xl md:text-5xl font-black md:font-light tracking-tighter uppercase font-heading">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 max-w-5xl w-full z-10 px-4 md:px-6">
        ...
        <motion.div
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
          className="glass-panel p-8 md:p-12 flex flex-col items-center gap-6 md:gap-8 group cursor-pointer border border-white/5 hover:border-cyan-neon/40 transition-all bg-white/[0.02] relative overflow-hidden"
          onClick={() => handleSelect('zenith')}
        >
          <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
            <Laptop className="w-24 h-24 -mr-8 -mt-8" />
          </div>
          <div className="w-16 h-16 md:w-20 md:h-20 bg-cyan-neon/10 rounded-2xl flex items-center justify-center group-hover:bg-cyan-neon/20 transition-colors border border-cyan-neon/20">
            <Laptop className="w-8 h-8 md:w-10 md:h-10 text-cyan-neon" />
          </div>
          <div className="text-center relative z-10">
